...
from virtex.data.readers import VideoCaptionReader


# fmt: off
parser = argparse.ArgumentParser("Serialize a video captions split to LMDB.")
parser.add_argument(
    "-d", "--data-root", default="datasets/phoenix",
    help="Path to the root directory of phoeniux dataset.",
)
parser.add_argument(
    "-s", "--csv", required = True,
...
parser.add_argument(
    "-o", "--output", default="datasets/serialized/phoenix_train.lmdb",
...
if __name__ == "__main__":
    ...
    dset = VideoCaptionReader(_A.data_root, _A.split)
    ...
    for idx in tqdm(range(len(dset))):
        ...
        instance = (instance["video_id"], video, instance["captions"])
